...
import { Badge, Button, Dropdown, Layout, Menu, Typography, message } from 'antd'
import { Input, List, Modal } from 'antd'
import { CloseOutlined, MessageOutlined, SearchOutlined } from '@ant-design/icons'
...
import { getAuthHeaders, getUserInfo, logout } from '../services/auth'
...
export function AppLayout() {
  ...
  const loadFeedbackItems = async (category: string, options?: { syncEditor?: boolean; silent?: boolean }) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/feedback?category=${encodeURIComponent(category)}`, {
  ...
  const handleConfirmFeedbackStatus = async (item: FeedbackItem, nextStatus: FeedbackStatus) => {
    ...
    setFeedbackSaving(true)
    try {
      const response = await fetch(`${API_BASE_URL}/api/feedback/${item.id}/status`, {
        method: 'PATCH',
        headers: {
          ...getAuthHeaders(),
          'Content-Type': 'application/json',
  ...
  const handleDeleteFeedback = (item: FeedbackItem) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        setFeedbackSaving(true)
        try {
          const response = await fetch(`${API_BASE_URL}/api/feedback/${item.id}`, {
            method: 'DELETE',
            headers: getAuthHeaders(),
          })
  ...
  return (
    ...
    <Layout style={{ height: '100vh', overflow: 'hidden' }}>
      ...
      <Modal
        ...
        closable={false}
        footer={[
          <Button key="add" onClick={handleAddFeedback}>
            {messages.common.add}
          </Button>,
          <Button key="save" type="primary" loading={feedbackSaving} onClick={handleSaveFeedback}>
            {messages.common.save}
          </Button>,
          <Button key="close" type="primary" onClick={handleCloseFeedback}>
            {messages.common.close}
          </Button>,
        ]}
        width={960}
      >
        <div style={{ display: 'grid', gridTemplateColumns: '250px 1fr', gap: 20, minHeight: 460 }}>
          <List
            bordered

...
export function SurveyTemplateDetailPage() {
  ...
  const { schema, sections, basePath } = useModuleData()
  ...
  const handleSave = () => {
    if (editingKey) {
      ...
      const optionsString = optionRows
        ...
        .join(',')
      setQuestions(questions.map((q) => (q.key === editingKey ? { ...q, ...editForm, options: optionsString } as QuestionRow : q)))
      setEditingKey(null)
      setEditForm({})
      setOptionRows([])
    }
  ...
  const handleDelete = (key: string) => {
    setQuestions(questions.filter((q) => q.key !== key).map((q, index) => ({ ...q, sortOrder: index + 1 })))
  }
  ...
  const handleMoveUp = (key: string) => {
    ...
    if (index > 0) {
      ...
      newQuestions[index] = { ...newQuestions[index - 1], sortOrder: temp.sortOrder }
      newQuestions[index - 1] = { ...temp, sortOrder: temp.sortOrder - 1 }
      setQuestions(newQuestions.sort((a, b) => a.sortOrder - b.sortOrder))
    }
  }
  ...
  if (!schema) {
    return <Card>未配置详情页：{pathname}</Card>
  }

...
I --> J[完成关闭（CLOSED_COMPLETED）]

E -->|否| K[问卷过期（SURVEY_EXPIRED）]
K --> L[更新回访状态（FOLLOW_UP_CLOSED）]
L --> J
```
...
| 当前状态     | 工单当前流程状态     | 是       |                                                                                                |
| 原工单编号   | 原始 / 关联工单编号  | 否       |                                                                                                |
| 消息通知模板 | 通知消息模板         | 否       | 在消息（通知短信）中选择自动或手动模板（具备模板搜索功能），点击发送后，消息会自动推送至客户。 |
| 电子邮件模板 | 邮件模板             | 否       |                                                                                                |
...
| 国家     | 客户国家    | 否       |                                                              |
| 邮箱     | 邮箱地址    | 否       |                                                              |
| 收货类型 | 门店 / 快递 | 是       | 当收货类型为“门店”且收货信息为门店时，应同时显示门店地址地图 |
| 收货地址 | 收货地址    | 否       | 进行 3PL 出库并传输至 SAP、WMS 系统时，地址信息应为门店地址  |
...
| 字段名称                | 说明                       | 是否必填 | 备注                                                 |
| ----------------------- | -------------------------- | -------- | ---------------------------------------------------- |
| 产品名称                | 受理对象名称               | 是       | 当受理对象为产品或配件时展示对应名称                 |
| 产品ID                  | 产品或配件唯一标识         | 是       | 与主数据保持一致                                     |
| 类别                    | 受理对象所属类别           | 是       | 如产品、配件等；当客户订购配件时自动归类为“配件”     |
| 库存状态                | 当前库存状态信息           | 否       | 由主数据自动同步                                     |
| 上市日期                | 产品上市时间               | 否       | 由主数据自动同步                                     |
| 生产工厂                | 产品所属生产工厂           | 否       | 若为双工厂生产，默认第一工厂，可通过下拉框手动修改。 |
| 电镀 / 焊接维修是否可行 | 是否支持电镀或焊接维修处理 | 否       | 用于辅助维修判定，可展示为Y/N                        |
| 配件名称                | 对应配件名称               | 否       | 当受理对象为配件或涉及配件时展示                     |
| 配件存放位置            | 配件当前存放位置           | 否       | 由主数据自动同步                                     |
...
| 字段名称         | 说明               | 是否必填 | 备注                   |
| ---------------- | ------------------ | -------- | ---------------------- |
| 申请负责人       | 提交申请的人员信息 | 是       | 从工单中复制           |
| 产品名称         | 申请的产品名称     | 是       | 从工单中复制           |
| 申请数量         | 本次申请的产品数量 | 是       | 默认为1                |
| 申请原因（大类） | 申请原因一级分类   | 是       | 如：维修、补货、调拨等 |
| 申请原因（中类） | 申请原因二级分类   | 是       | 依赖大类联动选择       |
...
| 支付流水号   | 支付凭证编号     | 否       |                                  |
| 最终支付请求 | 最终支付请求标识 | 否       |                                  |
| 支付截止日期 | 支付时限         | 否       | 自入库日起自动计算并填写 D+29 日 |
| 支付链接     | 支付入口链接     | 否       |                                  |
| 是否取消支付 | 是否取消支付     | 否       | 可手动取消支付                   |
...
| 产品名称     | 产品名称         | 是       |                                                        |
| 产品类别     | 产品分类         | 是       |                                                        |
| 生产工厂1    | 生产来源         | 否       | 工厂缺省时可手动选择工厂                               |
| 生产工厂2    | 生产来源         | 否       | 工厂缺省时可手动选择工厂                               |
| 生产工厂3    | 生产来源         | 否       | 工厂缺省时可手动选择工厂                               |
| 上市日期     | 产品上市时间     | 否       |                                                        |
| 零件保有期限 | 维修配件保有周期 | 否       |                                                        |
| 库存存放位置 | 库存位置         | 是       | 可编辑维护                                             |
| 安全库存状态 | 库存是否低于阈值 | 是       | 当SAP库存数量少于5件时显示为“缺货”，5件及以上显示为“-” |
| PS库存数量   | 系统库存数量     | 是       |                                                        |
| 3PL库存数量  | 第三方库存数量   | 是       |                                                        |
...
| 字段名称     | 说明                       | 是否必填 | 备注                                                                        |
| ------------ | -------------------------- | -------- | --------------------------------------------------------------------------- |
| 工单编号     | 关联的售后工单编号         | 是       | 跳转至对应的工单页面                                                        |
| 客户名称     | 客户姓名                   | 是       |                                                                             |
| 客户反应     | 客户反馈或问题描述         | 是       | 客户感动、客户满意、中立、不满、强烈不满                                    |
| 咨询分类     | 咨询类型分类               | 是       | 枚举                                                                        |
| 例外处理分类 | 是否属于异常或特殊处理类型 | 否       | 维修费用/配送周期/同款产品更换/更换其他产品<br>/镜片补偿/折旧/产品退款/其他 |
| 咨询负责人   | 负责处理咨询的人员         | 是       |                                                                             |
| 咨询日期     | 咨询发生时间               | 是       |                                                                             |
| 状态         | 咨询处理状态               | 是       | 如：待处理 / 已完成                                                         |
| 操作         | 查看 / 编辑                | 否       |                                                                             |

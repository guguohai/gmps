...
承接工单支付阶段的支付订单、支付状态、支付时间、支付链接及支付相关结果。
...
客户确认事项作为咨询事项的一种类型管理，当前不单独拆分独立业务对象，也不作为工单主流程的强制阻塞状态。
...
| 外呼类型     | outbound_type           | VARCHAR(64)  | 否   | NULL              | 字典维护                  | 外呼类型                |
| 咨询渠道     | consultation_channel    | VARCHAR(64)  | 否   | NULL              | 电话/线下/邮件/小程序          | 咨询渠道                |
| 咨询分类     | consultation_category   | VARCHAR(64)  | 否   | NULL              | 字典维护                  | 咨询分类                |
| 咨询需求分类   | consultation_demand_category | VARCHAR(64)  | 否   | NULL              | 字典维护                  | 咨询需求场景分类            |
| 事项主题     | subject                 | VARCHAR(200) | 否   | NULL              | 前台展示标题                | 事项主题                |
| 事项内容     | content                 | TEXT         | 否   | NULL              | 正文内容                  | 事项内容                |
| 事项状态     | status                  | VARCHAR(64)  | 是   | 无                 | 待处理/处理中/待客户确认/已完成/已关闭 | 事项自身状态，不直接驱动工单主流程状态 |
| 是否需要客户确认 | need_customer_confirm   | TINYINT      | 是   | 0                 | 0否1是                  | 是否需要客户确认            |
| 客户确认结果   | customer_confirm_result | VARCHAR(64)  | 否   | NULL              | 仅客户确认事项使用             | 客户确认结果              |
| 发起时间     | launched_at             | DATETIME     | 否   | NULL              | 创建事项时写入               | 发起时间                |
| 确认截止时间   | confirm_deadline        | DATETIME     | 否   | NULL              | 客户确认场景使用              | 确认截止时间              |
| 确认时间     | confirmed_at            | DATETIME     | 否   | NULL              | 客户确认时写入               | 确认时间                |
| 负责人ID    | owner_id                | BIGINT       | 否   | NULL              | 关联 sys_user.id        | 事项负责人               |
| 确认载荷     | confirm_payload_json    | TEXT         | 否   | NULL              | 客户确认内容快照              | 确认内容快照              |
| 确认通知次数   | confirm_notice_count    | INT          | 是   | 0                 | 通知后累加                 | 通知次数                |
| 处理结果     | process_result          | TEXT         | 否   | NULL              | 处理完成后填写               | 处理结果                |
| 是否阻塞主流程  | is_blocking_main_flow   | TINYINT      | 是   | 0                 | 0否1是                  | 是否阻塞工单推进            |
| 创建时间     | created_at              | DATETIME     | 是   | CURRENT_TIMESTAMP | 创建时写入                 | 创建时间                |
| 更新时间     | updated_at              | DATETIME     | 是   | CURRENT_TIMESTAMP | 更新时自动刷新               | 更新时间                |
...
|字段名称|字段Key|数据类型|必填|默认值|逻辑规则|说明|
|---|---|---|---|---|---|---|
|主键ID|id|BIGINT|是|无|主键|进展主键|
|工单ID|ticket_id|BIGINT|是|无|关联 ticket.id|所属工单|
|进展时间|progress_time|DATETIME|是|无|业务节点发生时写入|进展时间|
|进展类型|progress_type|VARCHAR(64)|是|无|受理/收件/待支付/发货等|进展类型|
|进展内容|progress_content|TEXT|否|NULL|面向客户可读描述|进展内容|
...
当前版本除工单主流程外，`INVENTORY_REQUEST`、`PART_REQUEST`、`CONSULTATION`、`SURVEY` 暂不强制初始化自动规则。

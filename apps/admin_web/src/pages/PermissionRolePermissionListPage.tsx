...
import { getPermissionRolePermissionList } from '../services/permissionApi'
...
export function PermissionRolePermissionListPage() {
  ...
  useEffect(() => {
    let cancelled = false
    getPermissionRolePermissionList()
  ...
  const columns = useMemo<ColumnsType<ModuleRow>>(() => [
    ...
    { title: '权限数', dataIndex: 'permCount', key: 'permCount', width: 100 },
    { title: '权限范围', dataIndex: 'permScope', key: 'permScope', width: 240 },
      ...
      render: (_, row: ModuleRow) => (
        <Button
          ...
          onClick={(event) => {
            ...
            navigate('/permission/role-permission/' + String(row.id))
          }}
        />
      ),
    },
  ...
  return (
    <div className="app-page-stack">
      <Row className="app-sticky-page-bar" justify="space-between" align="middle">
        <Col><Breadcrumb items={[{ title: messages.navigation.permission }, { title: messages.navigation.role_permission }]} /></Col>
        <Col><ListPageToolbar pathname="/permission/role-permission" /></Col>
      </Row>

...
import { getServiceSurveyDetail, getServiceSurveySections } from '../services/serviceApi'
...
const renderEditableField = (field: ModuleField) => {
  ...
  if (type === 'date') return <DatePicker style={{ width: '100%' }} defaultValue={parseDateValue(value)} />
...
export function SurveyDetailPage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    setLoading(true)
    Promise.all([getServiceSurveyDetail(id), getServiceSurveySections()])
  ...
  const surveyNo = String(currentRow?.surveyNo ?? '')
  const surveyStatus = String(currentRow?.status ?? '')
  ...
  const basicSections = useMemo(() => {
    ...
    const rowValueMap: Record<string, string> = {
      工单编号: String(currentRow?.ticketNo ?? ''),
      问卷编号: surveyNo,
      问卷模板: String(currentRow?.template ?? ''),
      问卷标题: String(currentRow?.title ?? ''),
      问卷说明: String(currentRow?.surveyDesc ?? ''),
      问卷状态: surveyStatus,
      截止时间: String(currentRow?.deadline ?? ''),
      是否允许提交: String(currentRow?.submittable ?? ''),
      题目数量: String(currentRow?.questionCount ?? surveyResultData.length),
      创建时间: String(currentRow?.createTime ?? ''),
      更新时间: String(currentRow?.updatedAt ?? ''),
    }

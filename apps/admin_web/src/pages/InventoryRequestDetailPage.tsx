...
import { getInventoryRequestDetail, getInventoryRequestSections } from '../services/inventoryApi'
...
type SectionList = ModuleSection[]
...
export function InventoryRequestDetailPage() {
  ...
  const basePath = '/inventory/request'
  ...
  useEffect(() => {
    ...
    let cancelled = false
    setLoading(true)
    Promise.all([getInventoryRequestDetail(id), getInventoryRequestSections()])
      .then(([detailRes, sectionRes]) => {
        if (cancelled) return
        const detailRow = detailRes.data ?? null
        const mappedSections = (sectionRes.data ?? []).map((section) => ({
          ...section,
          fields: section.fields.map((field) => {
            if (!detailRow) return field
